...
export function createProject(input: {
   ...
   * on a document that arrived from an import with no imprint. Resolved flow
   * copy goes on last — it is only safe once normalisation has run. */
  const stored = applyResolvedFlowCopy(normalizeArchitecture(hydrateImprint(doc)));
